...
function AjoutEntraineur() {
    ...
    return (
        <div>
            <h1 style={{ color: 'white' }}>AJOUT ENTRAINEUR</h1>
            <form onSubmit={handleSubmit}>
                <input type="text" placeholder="Nom" value={nom} onChange={handleNomChange} />
                <input type="text" placeholder="Prénom" value={prenom} onChange={handlePrenomChange} />
                <select value={sexe} onChange={handleSexeChange}>
                    <option value="">Sélectionnez votre sexe</option>
                    <option value="M">Monsieur</option>
                    <option value="F">Madame</option>
                </select>
                <input type="file" accept=".png, .jpg, .jpeg" onChange={handlePhotoChange} />
                <input type="text" placeholder="Adresse" value={adresse} onChange={handleAdresseChange} />
                ...
                <textarea placeholder="Phrase d'accroche" value={phraseAccroche} onChange={handlePhraseAccrocheChange} />
                <button type="submit" disabled={!isValidEmail || !isValidPassword} style={{ backgroundColor: isValidEmail && isValidPassword ? 'orange' : 'grey' }}>Ajouter</button>
            </form>
        </div>
    );
}
