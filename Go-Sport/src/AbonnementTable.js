...
function AbonnementTable() {
  ...
  const deleteAbonnement = async (id) => {
    ...
    setAbonnements(
      abonnements.filter((abonnement) => abonnement.id_abonnement !== id)
    );
  };
    ...
    <div id="abonnement">
      <div className="abonnement">
          ...
          <tbody>
            {filteredAbonnements.map((abonnement) => (
              <tr key={abonnement.id_abonnement}>
                ...
                <td>
                  <button
                    onClick={() => deleteAbonnement(abonnement.id_abonnement)}
                  >
                    Supprimer
                  </button>
                </td>
        ...
        <Link to="/AjoutAbonnement">
          <button id="ajout-ab">Ajouter un abonnement</button>
        </Link>
      </div>
    </div>
  );
