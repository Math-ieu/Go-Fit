...
function Inscription({ setPage }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'white' }}>Vous avez déjà un compte ? <span style={{ color: 'orange' }} onClick={() => setPage('login')}>Connectez-vous</span></p>
    </div>
  );
}
...
function Connexion({ setPage, setUser }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'white' }}>Vous êtes nouveau ? <span style={{ color: 'orange' }} onClick={() => setPage('register')}>Inscrivez-vous</span></p>
    </div>
  );
}
      ...
      <Box className="overlay">
        {page === 'home' && (
          <VStack spacing={5}>
            ...
            <Button colorScheme="teal" onClick={handleRegisterRedirect}>JE CRÉE MON COMPTE</Button>
